...
def process_ICMP_message(us,header,data,srcIp):
    ...
    logging.debug("Type: %d"%tyype)
    code = data[1]
    logging.debug("Code: %d"%code)

    if tyype == ICMP_ECHO_REQUEST_TYPE:
        message = data[8:]
        actid = struct.unpack('!H', data[4:6])[0]
        actseq = struct.unpack('!H', data[6:8])[0]
        sendICMPMessage(message, ICMP_ECHO_REPLY_TYPE, code, actid, actseq, struct.unpack('!I',srcIp)[0])

    if tyype == ICMP_ECHO_REPLY_TYPE:
        message = data[8:]
        actid = struct.unpack('!H', data[4:6])[0]
        actseq = struct.unpack('!H', data[6:8])[0]
        with timeLock:
            envio = icmp_send_times[(struct.unpack('!I',srcIp)[0])+actid+actseq]
            recepción = header.ts.tv_sec
            res = float(recepción) - envio
            logging.debug("RTT:%d"%res)
