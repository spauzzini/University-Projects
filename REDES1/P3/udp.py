...
def process_UDP_datagram(us,header,data,srcIP):
    ...
    '''
    sourceport = data[0:2]
    logging.debug("Puerto de origen: " + str(sourceport))
    destport = data[2:4]
    logging.debug("Puerto de destino: " + str(destport))
...
def sendUDPDatagram(data,dstPort,dstIP):
    ...
    '''
    datagram = bytearray()
    cabecera = struct.pack('!H', getUDPSourcePort()) + struct.pack('!H', dstPort) + struct.pack('!H', len(data)+UDP_HLEN)
    checksum = bytes([0x00, 0x00])
    datagram = cabecera + checksum + data
    if sendIPDatagram(dstIP, datagram, UDP_PROTO) is True:
        return True
    else:
        return False
