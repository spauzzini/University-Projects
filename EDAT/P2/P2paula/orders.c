...
int Order_Open(){
    ...
    /* cambiar ret pq he cambiado shippeddate*/
    ...
    SQLBindCol(stmt, 1, SQL_C_CHAR, ordernumber, sizeof(ordernumber), NULL);
...
int Order_Range(){
    ...
    SQLPrepare(stmt, (SQLCHAR*) "select ordernumber, orderdate, shippeddate from orders where orderdate between ? and ? order by ordernumber;", SQL_NTS);
/*
FALTA HACER BIEN ESTE CODIGO
