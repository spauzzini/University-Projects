...
def processARPReply(data:bytes,MAC:bytes)->None:
    ...
    logging.debug('Función processARPRequest: termina correctamente')
...
def ARPResolution(ip:int) -> bytes:
    ...
    #TODO implementar aquí
    if ip in cache:
        print("La ip está en el caché, no se realiza una petición ARP\n")
        cacheLock.acquire()
        cache_info = cache[ip]
        cacheLock.release()
        logging.debug('Función ARPResolution: La ip existe en el caché')
        return cache_info
    ...
    frame = createARPRequest(requestedIP)
    print("Creando ARP request\n")
    for n in range(0, 3):
